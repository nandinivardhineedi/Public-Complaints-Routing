...
export const ComplaintList: React.FC<ComplaintListProps> = ({ complaints, onUpdateComplaintStatus }) => {
  ...
  const filteredComplaints = useMemo(() => {
    return complaints.filter(c => {
      // Search logic checks the display ID, falling back to the internal ID
              ...
              {paginatedComplaints.map((complaint) => (
                <tr key={complaint.id} className={`transition-colors duration-1000 ease-out ${newComplaintIds.has(complaint.id) ? 'bg-indigo-50' : 'hover:bg-gray-50'}`}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    {/* Display the sequential ID, fall back to internal ID */}
      ...
      <ComplaintDetailModal 
        ...
        onUpdateStatus={async (newStatus) => {
          if (selectedComplaint) {
            // Passes the internal Firestore ID ('id') which the backend uses to find the doc.
            await onUpdateComplaintStatus(selectedComplaint.id, newStatus); 
